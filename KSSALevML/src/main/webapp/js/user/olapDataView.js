...
let olapDataView = {
	    ...
	    condAddAction : function(condDataset){
	    	...
	    	let elemConfig = {
	    		condSetClass : "form-inline d-flex justify-content-left ml-2" ,
	    	...
	    	if(_flag === "draw" || _flag === "only"){
	    		...
	    		var options = {
	    		        ...
	    		        rowClick: function(args){
							...
							/**
							 2022-10-26
							 제약조건: AS_CNT을 사용자가 반드시 선택해야 한다. 
							 */
							 const selectedData = args.item;
							 if(selectedData.hasOwnProperty("AS_CNT") &&
							 $(args.event.target).hasClass("click-evt-as_cnt") // 사용자가 'AS_CNT 컬럼을 클릭했는지 여부
							 ){
								 if(selectedData.hasOwnProperty("BOM")){
									 const bom = selectedData["BOM"];
									 // modal 크기조정
					 				...
					 				$.ajax({ 
										type:"GET",
										url:OlapUrlConfig.selectASHistoryGridData, 
										 beforeSend: function(xhr) {
										        xhr.setRequestHeader("AJAX", true);
										     },
										     data :{
...
'</svg>';
											...
											CustomCollapseGrid("asHistoryGrid",collapseConfig,_result);
									...
									alert("AS 상세 이력을 확인하려면 '항목 선택'에서 'BOM' 컬럼을 추가하십시오.");
								 }
...
//	    				fields[i]["width"] =  "auto";
	    				...
	    				fields[i]["width"] =  setFieldWidth(field);
	    				// 2022-10-26 'AS 이력' 컬럼 값 클릭 시 이벤트 추가 
	    				if(field === "AS_CNT"){
							fields[i]["css"] += " click-evt-as_cnt";
						}
	    			}
	    ...
	    // 2022-10-26 event Function
	    asCntEvtAction : function(thisObj){
			console.log($(thisObj).text());
			const itemCd = $(thisObj).sibli
		}

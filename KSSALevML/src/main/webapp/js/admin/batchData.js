		...
		$("#step_jsGrid").jsGrid(
				{
					...
					rowClick: function(args){
						...
						if(rowData.hasOwnProperty("status") && rowData.status === "FAILED"){
							
							$("#failDetailPopup").modal('toggle');
							const windowInHeight = $(window).innerHeight();
							$("#failDetailPopup .modal-body .overflow-auto").height(windowInHeight - 400);
							$("#failDetailMsg").html($.parseHTML(rowData["exit_message"]));
						}
					},
					controller : {
...
let batchAction = { 
		...
		startAction : function(startDate,endDate,flag){
			confirm({

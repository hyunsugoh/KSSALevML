...
<script>
	...
	$(document).ready(function() {
		...
		$('#dataSubmitBtn').click(function(e){
			...
			var restr = $('#input_replace_string').val();
			if(chkData.length==0){
				alert('칼럼을 체크해 주세요.');
				return;
			}
			if(restr.length==0){
				alert('Replace String를 체크해 주세요.');
				return;
			}
			...
			$.doPost({
				url : "/mliframe/replaceMissingString.do",
				data : param,
				success : function(data){
	...
	<div class="col-3 card mr-10" id="div_control">
		...
		<div class="card-title mb-0">
			<div class="row">
				<div class="col">
					<p class="h5 mb-1">
						<label class="mliFrame-name required" id="label_replace_string"><strong>Replace String</strong></label>
					</p>
				</div>
			</div>
		</div>		
		<div class="form-group">
			<input type="text" class="form-control-text" id="input_replace_string" name="replace_string" />
		</div>
	</div>

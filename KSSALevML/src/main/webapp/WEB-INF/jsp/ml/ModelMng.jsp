...
$(document).ready(function(){
	...
	$("#btn_data_conversion").click(function(e){
		...
		$('#modalDataCnv').modal("show");
		
		$("#modalDataCnv_confirm_ok").click(function(){
		
			var loadingMsg = new loading_bar({message:"Data Conversion 진행중..."});
			$.ajax({
				type:"POST",
				//<c:url value='/ext/orgToCnv.do'/>"
				url:"<c:url value='/ext/orgToCnv.do'/>", //KSM용
				data:{
					start_dt : $("#startDate").val().replace(/-/gi, ""),
					end_dt : $("#endDate").val().replace(/-/gi, "")
				},
				headers: {'X-CSRF-TOKEN': $('#csrfvalue').val()},
				beforeSend: function(xhr) {
				       xhr.setRequestHeader("AJAX", true);
	...
	$("#dataCnvAllChecked").change(function(){
		if($("#dataCnvAllChecked").is(":checked")){
					...
					<div class="col-lg-2 text-center " >
						<div class="d-flex"  >
							<input id="datepicker_start" readonly
								...
								data-toggle="datepicker"   style="margin-right:5px;">
								<button id="datepickerStartBtn"
									class="btn btn-sm btn-outline-secondary  custom-datepicker-icon">
  ...
  <div class="modal-dialog modal-lg" role="document"  style="max-width:350px;">
    ...
    <div class="modal-content">
      ...
      <div class="modal-footer justify-content-center">
      	<button type="button" class="btn btn-outline-primary btn-md" data-dismiss="modal"  id="modalModelCreate_confirm_ok">진행 <i class="fas fa-plus ml-1"></i></button>
...
<!--Modal: Create Pop-->
<div class="modal fade" id="modalDataCnv" tabindex="-1" role="dialog"  aria-hidden="true" data-backdrop="static" data-keyboard="false">
  <div class="modal-dialog modal-lg" role="document"  style="max-width:350px;">
    <!--Content-->
    <div class="modal-content">
      <!--Body-->
      <div class="modal-body mb-0 p-1">
        <!--Google map-->
        <div id="map-container-google-2" class="z-depth-1-half map-container" style="height: 100px;padding:5px;">
			<p class="h6 pt-3 pb-4 text-center">
				<strong >실적 데이터 변환작업을 처리합니다. 처리대상의 기존 데이터는 삭제됩니다.</strong>
			</p>
        </div>
        ...
        <div class="row" style="padding:5px;">
			<div class="col-12">
				<div class="custom-control custom-checkbox " style="width: 30%; background: url();">
					<input type="checkbox" class="custom-control-input" id="dataCnvAllChecked" checked> 
					<label class="custom-control-label" for="dataCnvAllChecked" style="margin-top: -3px;"> 전체</label>
				</div>
			</div>
		</div>
		<div class="row"  style="padding:5px;">
			...
			<div class="col-6 ">
				<input type="text" name="startDate" id="startDate" readonly  placeholder="YYYYMMDD"  data-inputmask-alias="yyyy/mm/dd"/>
			</div>
		</div>		
		<div class="row"  style="padding:5px;">
	        <div class="col-6 ">
			...
			<div class="col-6 ">
				<input type="text" name="endDate" id="endDate" readonly placeholder="YYYYMMDD"/>
			</div>
		</div>		
      </div>
      <!--Footer-->
      <div class="modal-footer justify-content-center">
      	<button type="button" class="btn btn-outline-primary btn-md" data-dismiss="modal"  id="modalDataCnv_confirm_ok">진행 <i class="fas fa-plus ml-1"></i></button>
        <button type="button" class="btn btn-outline-secondary btn-md" data-dismiss="modal" >취소 <i class="fas fa-times ml-1"></i></button>
      </div>
    </div>
    <!--/.Content-->

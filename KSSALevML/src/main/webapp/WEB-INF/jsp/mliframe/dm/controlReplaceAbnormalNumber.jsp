...
<script>
	...
	$(document).ready(function() {
		...
		$('#dataSubmitBtn').click(function(e){
			...
			var renum = $('#input_replace_number').val();
			if(isNaN(abnum)){
				alert('Abnormal Number를 체크해 주세요.');
				return;
			}
			if(isNaN(renum)){
				alert('Replace Number를 체크해 주세요.');
				return;
			}
			if(abnum.length==0){
				alert('Abnormal Number를 체크해 주세요.');
			...
			$.doPost({
				url : "/mliframe/replaceMissingNumber.do",
				data : param,
				success : function(data){
	...
	<div class="col-3 card mr-10" id="div_control">
		...
		<div class="card-title mb-0">
			<div class="row">
				<div class="col">
					<p class="h5 mb-1">
						<label class="mliFrame-name required" id="label_abnormal_number"><strong>Abnormal Number</strong></label>
					</p>
				</div>
			</div>
		</div>		
		<div class="form-group">
			<input type="text" class="form-control-number" id="input_abnormal_number" name="abnormal_number" />
		</div>
		<div class="card-title mb-0">
			<div class="row">
				<div class="col">
					<p class="h5 mb-1">
						<label class="mliFrame-name required" id="label_replace_number"><strong>Replace Number</strong></label>
					</p>
				</div>
			</div>
		</div>		
		<div class="form-group">
			<input type="text" class="form-control-number" id="input_replace_number" name="replace_number"/>
		</div>
	</div>
